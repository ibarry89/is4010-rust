...
fn problem_2() {
    ...
    println!("  {}", r1); // r1 is used here
    let r2 = &mut s; // mutable borrow is ok now because r1 is done
    println!("  {}", r2);
}
...
fn create_string() -> String {
    String::from("hello") // Return owned String, not a reference
}
...
fn problem_6() {
    ...
    for i in 0..3 {
        print_with_number(&data, i); // Pass reference instead of moving
    }
}
...
fn problem_7() {
    ...
    let s = String::from("inner scope"); // Move outside the inner block
    let result = &s;
